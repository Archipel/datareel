...
__SBYTE__ gxDatabase::gxSignature[gxSignatureSize] = {'G', 'X', 'D', 'B', 'A', 'S', 'E', gxDatabaseRevisionLetter};
#endif
...
gxDatabaseError gxDatabase::Create(const char *fname, FAU_t static_size, __SBYTE__ RevisionLetter)
...
// letter errors when new database files are created.
{
	...
	switch (RevisionLetter) {
		case '\0':
		case ' ': // Database revision zero
			rev_letter = '\0';
			break;
		case 'A':
		case 'a': // Persistent checksum revision 
			rev_letter = 'A';
			break;
		case 'B':
		case 'b': // Persistent file lock revision
			rev_letter = 'B';
			break;
		case 'C':
		case 'c': // Persistent record lock revision
			rev_letter = 'C';
			break;
		case 'D':
		case 'd': // Persistent file stats with record locking
			rev_letter = 'D';
			break;
		case 'E':
		case 'e': // File stats without persistent CRC or record locking
			rev_letter = 'E';
			break;
		default:
			rev_letter = gxDatabaseRevisionLetter;
...
gxDatabaseError gxDatabase::Open(const char *fname, gxDatabaseAccessMode mode)
...
gxDatabaseError gxDatabase::Read(void *buf, __ULWORD__ bytes, FAU_t file_address)
...
gxDatabaseError gxDatabase::Write(const void *buf, __ULWORD__ bytes, FAU_t file_address, int flushdb, int bit_test)
...
// value to indicate an error condition or zero if successful.
{
	...
	if ((IsOK()) && (ReadyForWriting())) {
		...
		if (bit_test) {
			// Record the current write offest for bit testing
			if (file_address == gxCurrAddress)
...
int gxDatabase::ReadObjectChecksum(FAU_t object_address, __ULWORD__ *object_crc, __ULWORD__ *calc_crc)
...
// a file error occurs.
{
	...
	if (object_crc)
		*object_crc = objectCRC;
	if (calc_crc)
		*calc_crc = CRC;

	if (CRC ^ objectCRC)
		return 0; // Return false if CRC check fails

	return 1; // Return true if the CRC values match
}

__ULWORD__ gxDatabase::CalcChecksum(__ULWORD__ bytes, FAU_t file_address, int mem_alloc)
...
// Returns the CRC value.
{
	...
	if (buf) {
		...
		if (IsOK()) {
			...
			CRC = 0xffffffffL;
			while (len--) {
				if (Read(&data, sizeof(data)) != gxDBASE_NO_ERROR)
					return 0;
...
gxDatabaseError gxDatabase::ReadBlockHdr(gxBlockHeader &hdr, FAU_t block_address)
...
gxDatabaseError gxDatabase::WriteBlockHdr(const gxBlockHeader &hdr, FAU_t block_address)
...
// header is read.
{
	if (Read(&file_header, sizeof(gxFileHeader), gxStartOfFile) != gxDBASE_NO_ERROR) {
		return gxd_error;
	}
...
{
	if (Write(&file_header, sizeof(gxFileHeader), gxStartOfFile) != gxDBASE_NO_ERROR) {
		return gxd_error;
	}
	switch (rev_letter) {
		case 'd':
...
// block was not deleted/removed.
{
	...
	if (ReadyForWriting()) {
		...
		if (WriteBlockHdr(blk_hdr, block_address) != gxDBASE_NO_ERROR)
			return 0;
...
// Returns the total number of valid blocks in the file.
{
	...
	while (block_address) {
		...
		if (block_address >= file_header.gxd_eof)
			break;
...
// Undeletes a block if it has not been removed or reclaimed.
{
	...
	while (addr) {
		...
		if (addr == block_address) {
			...
			// Write header for this block
			if (WriteBlockHdr(blk_hdr, addr) != gxDBASE_NO_ERROR)
				return 0;
...
// plus any block overhead plus one byte.
{
	...
	while (addr) {
		...
		if (unused_len > 0) {
			// Found bigger block with room for header
			for (i = 0; i < max_limit; i++) {
...
// end of file is reached.
{
	...
	if (next_blk >= file_header.gxd_eof)
		return file_address;
...
// is currently updating it. 
{
	...
	if (ReadFileLockHdr(lock_header) != gxDBASE_NO_ERROR)
		return gxd_error;
...
gxDatabaseError gxDatabase::ReadRecordLockHdr(gxRecordLockHeader &hdr, FAU_t block_address)
...
gxDatabaseError gxDatabase::WriteRecordLockHdr(const gxRecordLockHeader &hdr, FAU_t block_address)
...
// to indicate an error condition or zero if successful.
{
	...
	return Write(&hdr, sizeof(gxFileStatsHeader), (sizeof(gxFileHeader) + sizeof(gxFileLockHeader)));
}
...
// indicate an error condition or zero if successful.
{
	...
	return Read(&hdr, sizeof(gxFileStatsHeader), (sizeof(gxFileHeader) + sizeof(gxFileLockHeader)));
}

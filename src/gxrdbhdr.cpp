...
const char *gxrdFIELD_TYPES_STRINGS[gxrdNUM_FIELD_TYPES] = {"Invalid type", // gxrdINVALID_TYPE
                                                            "CHAR", // gxrdCHAR 
                                                            "INT16", // gxrdINT16 
                                                            "INT32", // gxrdINT32
                                                            "INT64", // gxrdINT64
...
int gxRDBMSHeader::TestFileID(const gxRDBMSHeader &ob) const {
	if (memcmp(file_id, ob.file_id, (gxrdFILE_ID_STR_LEN - 1)) != 0)
		return 0;
	return 1;
}

int gxRDBMSHeader::TestVersion(const gxRDBMSHeader &ob) const {
	if (file_version != ob.file_version)
		return 0;
	return 1;
}
...
int gxRDBMSHeader::TestHeader(const gxRDBMSHeader &ob) const {
	if (memcmp(file_id, ob.file_id, gxrdFILE_ID_STR_LEN) != 0)
		return 0;
	if (file_version != ob.file_version)
		return 0;
	return 1;
...
void gxrdRecordHeader::SetTableID(const gxString &table_name) {
	if (table_name.is_null())
		table_id = static_cast<gxUINT32>(0);
...
int gxrdTableHeaderField::TestFieldType(const gxrdTableHeaderField &ob, int strict_test) const {
	if (strict_test) {
	...
	if (field_type_id != ob.field_type_id) {
		...
		switch (type1) {
			...
			case gxrdINT32:
				...
				return 0;
			case gxrdINT64:
				if ((type2 == gxrdINT16) || (type2 == gxrdINT32) || (type2 == gxrdINT64)) {
			...
			case gxrdBINARY:
				if (type2 != gxrdBINARY)
					return 0; // Cannot cast user-defined type
				break;
			case gxrdDATE:
				if (type2 != gxrdDATE)
					return 0; // Cannot cast date type
				break;
			default:
				return 0;
		}
...
int gxrdTableHeaderField::TestFieldVars(const gxrdTableHeaderField &ob) const {
	if (format_specifier != ob.format_specifier)
		return 0;
	if (array_length != ob.array_length)
		return 0;
	if (column_length != ob.column_length)
		return 0;
...
// store in the table header.
{
	...
	for (__ULWORD__ i = 0; i < num_fields; i++) {
		...
		def << fields[i].field_name << "{" << fields[i].column_length << "}" << gxrdFieldTypeString(static_cast<gxrdFIELD_TYPES>(type));
...
int gxrdTableHeader::VerifyFieldTypes() const {
	if (num_fields == static_cast<__ULWORD__>(0))
		return 0;
	if (!fields)
		return 0; // Object is not initialized

	for (__ULWORD__ i = 0; i < num_fields; i++) {
		if (!fields[i].VerifyFieldType())
			return 0;
	}
	return 1;
}
...
int gxrdTableHeader::TestFieldTypes(const gxrdTableHeader &ob) const {
	if (num_fields != ob.num_fields)
		return 0;
	if (fields) {
		if (!ob.fields)
			return 0; // Object is not initialized
		for (__ULWORD__ i = 0; i < num_fields; i++) {
			if (!fields[i].TestFieldType(ob.fields[i]))
				return 0;
		}
	}
	if (ob.fields) {
...
int gxrdTableHeader::TestFieldVars(const gxrdTableHeader &ob) const {
	...
	if (fields) {
		if (!ob.fields)
			return 0; // Object is not initialized
		for (__ULWORD__ i = 0; i < num_fields; i++) {
			if (!fields[i].TestFieldVars(ob.fields[i]))
				return 0;
		}
	}
	if (ob.fields) {
...
int gxrdTableHeader::AllocHeader(__ULWORD__ nfields) {
	...
	if (nfields <= static_cast<gxUINT32>(0))
		nfields = tableMIN_FIELDS;
	if (nfields > static_cast<__ULWORD__>(tableMAX_FIELDS))
		nfields = tableMAX_FIELDS;
...
int gxrdTableHeader::SetField(char field_type_id, char format_specifier, char *field_name, const gxUINT32 &column_length, const gxUINT32 &array_length, __ULWORD__ nfield) const {
	if (!fields)
		return 0;
	if (nfield > (num_fields - 1))
		return 0;
...
gxDatabaseError gxrdDatabaseTable::CreateTable(const gxString &tdef) {
	...
	for (i = 1; i < num_words; i++) {
		if (*words[i]) {
			...
			table_header.SetField(field_type_id, format_specifier, field_name.c_str(), column_length, alen, (i - 1));
...
int gxrdDatabaseKey::operator==(const DatabaseKeyB &k) const {
	...
	if (!allow_dup_names) {
		...
		return ((key.k == kptr->key.k) && (key.kid == kptr->key.kid));
	}
}
...
{
	if ((!db_record) || (!num_fields))
		return static_cast<__ULWORD__>(0);
	...
	for (__ULWORD__ i = 0; i < num_fields; i++) {
		...
		memmove(&num_bytes, (db_record[i].m_buf() + field_type_size), sizeof(num_bytes));
...
__ULWORD__ gxrdDatabaseRecord::SizeOfField(__ULWORD__ col_number) const {
	...
	total_size += (sizeof(num_bytes) + field_type_size);
	memmove(&num_bytes, (db_record[col_number].m_buf() + field_type_size), sizeof(num_bytes));
...
void *gxrdDatabaseRecord::FieldData(__ULWORD__ col_number, char &field_type_id, gxUINT32 &num_bytes) const {
	if (!db_record)
		return static_cast<void *>(nullptr);
	if (col_number > (num_fields - 1))
		return static_cast<void *>(nullptr);
	__ULWORD__ offset = 0;
	memmove(&field_type_id, (db_record[col_number].m_buf() + offset), sizeof(field_type_id));
	offset += sizeof(field_type_id);
	memmove(&num_bytes, (db_record[col_number].m_buf() + offset), sizeof(num_bytes));
...
gxDatabaseError gxrdDatabaseRecord::FieldDataDiskOffset(__ULWORD__ &offset, __ULWORD__ col_number) const {
...
gxDatabaseError gxrdDatabaseRecord::FieldDiskOffset(__ULWORD__ &offset, __ULWORD__ col_number) const {
	if (!db_record)
	...
	for (__ULWORD__ i = 0; i < col_number; i++) {
		offset += 1; // Field type ID
		memmove(&num_bytes, (db_record[i].m_buf() + 1), sizeof(num_bytes));
		offset += sizeof(num_bytes);
		offset += num_bytes;
	}
...
gxDatabaseError gxrdDatabaseRecord::GetField(void *data, __ULWORD__ bytes, __ULWORD__ col_number) const {
	if (!db_record)
		return gxDBASE_NULL_PTR;
	if (col_number > (num_fields - 1))
		return gxDBASE_BUFFER_OVERFLOW;
...
void *gxrdDatabaseRecord::GetField(__ULWORD__ col_number, __ULWORD__ &bytes, gxDatabaseError &err) const {
	if (!db_record) {
...
gxDatabaseError gxrdDatabaseRecord::SetField(const void *data, __ULWORD__ bytes, __ULWORD__ col_number) const {
	if (!db_record)
		return gxDBASE_NULL_PTR;
	if (col_number > (num_fields - 1))
		return gxDBASE_BUFFER_OVERFLOW;
...
void gxrdDatabaseRecord::GetRecordTypeSize(gxrdFIELD_TYPES type, __ULWORD__ &size, __ULWORD__ &overhead) {
...
// successful or false if a memory allocation error occurs.
{
	...
	if (nfields <= 0)
		nfields = recordMIN_FIELDS;
	if (nfields > static_cast<__ULWORD__>(recordMAX_FIELDS))
		nfields = recordMAX_FIELDS;
...
gxDatabaseError gxrdDatabaseRecord::InitRecord(const gxString &def) {
	...
	for (int i = 1; i < num_words; i++) {
		if (*words[i]) {
			...
			__ULWORD__ offset = 0;
			memmove((db_record[(i - 1)].m_buf() + offset), &field_type_id, sizeof(field_type_id));
			offset += sizeof(field_type_id);
			memmove((db_record[(i - 1)].m_buf() + offset), &num_bytes, sizeof(num_bytes));
		}
		else {
			return gxDBASE_BAD_FIELD_TYPE;
		}
...
gxrdKeyType_t gxrdDatabaseRecord::CreateKey(__ULWORD__ field, int case_cmp) const {
	if (!db_record)
		return static_cast<gxrdKeyType_t>(0);

	int compare_all = 0;
	if (field == static_cast<__ULWORD__>(-1))
		compare_all = 1;
	if (field > num_fields)
		compare_all = 1;
  ...
  gxINT64 i64buf;
	...
	for (; i < num_fields; i++) {
		offset = 0;
		npos = 0;
		memmove(&field_type_id, (db_record[i].m_buf() + offset), sizeof(field_type_id));
		offset += sizeof(field_type_id);
		memmove(&num_bytes, (db_record[i].m_buf() + offset), sizeof(num_bytes));
		offset += sizeof(num_bytes);
		...
		switch (type) {
			...
			case gxrdVCHAR:
				...
				if (npos == 0) {
					...
					sbuf.InsertAt(0, reinterpret_cast<char *>(db_record[i].m_buf() + offset), num_bytes);
				}
				else {
			...
			case gxrdBINARY: // User defined types and small binary objects
				if (num_bytes < sizeof(char))
					return static_cast<gxrdKeyType_t>(0);
				bin_buf.Clear();
				bin_buf.InsertAt(0, reinterpret_cast<char *>(db_record[i].m_buf() + offset), num_bytes);
...
int gxrdDatabaseRecord::Compare(const gxrdDatabaseRecord &ob, __ULWORD__ field, int case_cmp, int alpha_cmp, int find_match) const {
	int result = -1;
	gxString result_buf;
	if ((!db_record) || (!ob.db_record))
		return result;
	if (table_name != ob.table_name)
		return result;
	if (record_def != ob.record_def)
		return result;
	if (num_fields != ob.num_fields)
		return result;
	...
	if (field == static_cast<__ULWORD__>(-1))
		compare_all = 1;
	if ((field > num_fields) || (field > ob.num_fields))
		compare_all = 1;
  ...
  gxINT64 i64buf1, i64buf2;
	...
	for (; i < num_fields; i++) {
		offset1 = offset2 = 0;
		npos1 = npos2 = 0;
		memmove(&field_type_id1, (db_record[i].m_buf() + offset1), sizeof(field_type_id1));
		offset1 += sizeof(field_type_id1);
		memmove(&num_bytes1, (db_record[i].m_buf() + offset1), sizeof(num_bytes1));
		offset1 += sizeof(num_bytes1);

		memmove(&field_type_id2, (ob.db_record[i].m_buf() + offset2), sizeof(field_type_id2));
		offset2 += sizeof(field_type_id2);
		memmove(&num_bytes2, (ob.db_record[i].m_buf() + offset2), sizeof(num_bytes2));
		offset2 += sizeof(num_bytes2);

		// Two different types specified
		if (field_type_id1 != field_type_id2)
			return -1;
		...
		switch (type) {
			...
			case gxrdVCHAR:
				...
				for (j = 0; j < num_bytes2; j++, p2++) {
					if (*p2 == 0) {
						npos2 = j;
						if (j == 0)
							npos2++; // The first char is null
						break;
					}
				}
				if ((npos1 == 0) || (npos2 == 0)) {
					sbuf1.InsertAt(0, reinterpret_cast<char *>(db_record[i].m_buf() + offset1), num_bytes1);
					sbuf2.InsertAt(0, reinterpret_cast<char *>(ob.db_record[i].m_buf() + offset2), num_bytes2);
				}
				else {
			...
			case gxrdBINARY: // User defined types and small binary objects
				...
				bin_buf1.Clear();
				bin_buf2.Clear();
				bin_buf1.InsertAt(0, reinterpret_cast<char *>(db_record[i].m_buf() + offset1), num_bytes1);
				bin_buf2.InsertAt(0, reinterpret_cast<char *>(ob.db_record[i].m_buf() + offset2), num_bytes2);

				if (find_match) {
					// Search for matching byte pattern
	...
	if(i64buf1 > i64buf2) result = 1;
			...
			case gxrdFLOAT:
			case gxrdCURRENCY:
				if ((num_bytes1 < sizeof(gxFLOAT64)) || (num_bytes2 < sizeof(gxFLOAT64))) {
					return -1;
				}
				if (find_match) {
					// Search for matching byte pattern
		...
		if (compare_all) {
			...
			// Compare a single field and exit
			return result;
		}
	}

	// A record field is less then the total weight
	if (result_buf.Find("-1"))
		return -1;
...
gxDatabaseError gxrdDatabaseRecord::CreateRecord(const gxString &def, __ULWORD__ cmp_field, int case_cmp, int alpha_cmp) {
...
gxDatabaseError gxrdDatabaseRecord::CreateDiskRecord(MemoryBuffer &disk_record) const {
	...
	for (__ULWORD__ i = 0; i < num_fields; i++) {
		...
		memmove(&field_type_id, (db_record[i].m_buf() + rec_offset), sizeof(field_type_id));
		rec_offset += sizeof(field_type_id);
		memmove(&num_bytes, (db_record[i].m_buf() + rec_offset), sizeof(num_bytes));
		rec_offset += sizeof(num_bytes);

		memmove((disk_record.m_buf() + buf_offset), &field_type_id, sizeof(field_type_id));
		...
		if (num_bytes == static_cast<gxUINT32>(0))
			continue;
		...
		memmove((disk_record.m_buf() + buf_offset), (db_record[i].m_buf() + rec_offset), num_bytes);
...
gxDatabaseError gxrdDatabaseRecord::LoadDiskRecord(const MemoryBuffer &disk_record) const {
	if (!db_record)
		return gxDBASE_NULL_PTR;
	...
	for (__ULWORD__ i = 0; i < rh.num_fields; i++) {
		...
		memmove(&field_type_id, (disk_record.m_buf() + buf_offset), sizeof(field_type_id));
		buf_offset += sizeof(field_type_id);
		byte_count += sizeof(num_bytes);
		if (byte_count > disk_record.length()) {
			return gxDBASE_BAD_RECORD;
		}
		memmove(&num_bytes, (disk_record.m_buf() + buf_offset), sizeof(num_bytes));
		buf_offset += sizeof(num_bytes);
		...
		__ULWORD__ rec_offset = 0; // Reset the record offset
		memmove((db_record[i].m_buf() + rec_offset), &field_type_id, sizeof(field_type_id));
		rec_offset += sizeof(field_type_id);
		...
		memmove((db_record[i].m_buf() + rec_offset), &num_bytes, sizeof(num_bytes));
		rec_offset += sizeof(num_bytes);
		...
		memmove((db_record[i].m_buf() + rec_offset), (disk_record.m_buf() + buf_offset), num_bytes);
		buf_offset += num_bytes; // Position the buffer offset to the next field
	}
...
void gxrdExcludeFields::SetNumFields(gxUINT32 nfields) {
	if (nfields < tableMIN_FIELDS)
		nfields = tableMIN_FIELDS;
	if (nfields > tableMAX_FIELDS)
		nfields = tableMAX_FIELDS;
	delete fields;
	num_fields = nfields;
...
GXDLCODE_API const char *gxrdFieldTypeString(gxrdFIELD_TYPES t) {
	...
	if (error > (gxrdNUM_FIELD_TYPES - 1))
		error = gxrdINVALID_TYPE;
